...
const RACKET_HEIGTH: f32 = 160.0;
const RACKET_WIDTH: f32 = 20.0;
const RACKET_HEIGTH_HALF: f32 = RACKET_HEIGTH * 0.5;
...
fn move_racket(pos: &mut na::Point2<f32>, keycode: KeyCode, y_dir: f32, ctx: &mut Context){
    ...
    clamp(&mut pos.y, RACKET_HEIGTH_HALF, screen_h - RACKET_HEIGTH_HALF);
}
...
impl event::EventHandler for MainState{
    fn update(&mut self, ctx: &mut Context) -> GameResult {
        ...
        let intersetcs_player_1 = 
            self.ball_pos.x - BALL_SIZE_HALF < self.player_1_pos.x + RACKET_WIDTH_HALF
            && self.ball_pos.x + BALL_SIZE_HALF > self.player_1_pos.x - RACKET_WIDTH_HALF
            && self.ball_pos.y - BALL_SIZE_HALF < self.player_1_pos.y + RACKET_HEIGTH_HALF
            && self.ball_pos.y - BALL_SIZE_HALF > self.player_1_pos.y - RACKET_HEIGTH_HALF;
        ...
        let intersetcs_player_2 = 
            self.ball_pos.x - BALL_SIZE_HALF < self.player_2_pos.x + RACKET_WIDTH_HALF
            && self.ball_pos.x + BALL_SIZE_HALF > self.player_2_pos.x - RACKET_WIDTH_HALF
            && self.ball_pos.y - BALL_SIZE_HALF < self.player_2_pos.y + RACKET_HEIGTH_HALF
            && self.ball_pos.y - BALL_SIZE_HALF > self.player_2_pos.y - RACKET_HEIGTH_HALF;
    ...
    fn draw(&mut self, ctx: &mut Context) -> GameResult {
        ...
        let racket_rect = graphics::Rect::new(-RACKET_WIDTH_HALF, -RACKET_HEIGTH_HALF, RACKET_WIDTH, RACKET_HEIGTH);
        ...
        draw_param.dest = [screen_w * 0.5, 0.0].into();
        ...
        let mut score_pos = na::Point2::new(screen_w * 0.5, 40.0);
...
fn main() -> GameResult{
    let cb = ContextBuilder::new("PP", "ASS");
    let (mut ctx, mut event_loop ) = cb.build()?;
    graphics::set_window_title(&ctx, "GGEZ");
